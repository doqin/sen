...
bool Lexer::isAlpha(const std::string& utf8Char) {
    ...
    if ((firstByte >= 'a' && firstByte <= 'z') || (firstByte >= 'A' && firstByte <= 'Z') || firstByte == '_') {
        return true;
    }
...
int Lexer::utf8CharSize(unsigned char firstByte) {
    if ((firstByte & 0x80) == 0) return 1; // 1-byte (ASCII)
...
std::string Lexer::extractUtf8Char(const std::string& text, size_t& pos) {
    if (pos >= text.length()) return ""; // Prevent out-of-bounds access

    int charSize = utf8CharSize(static_cast<unsigned char>(text[pos]));
    
    // Prevent reading beyond the string length
    if (pos + charSize > text.length()) charSize = text.length() - pos;
...
Token Lexer::nextToken() {
    ...
    if (pos >= source.length()) return { TokenType::EndOfFile, ""};
    
    size_t startPos = pos;
    std::string firstChar = extractUtf8Char(source, pos);  // Note: pos is passed as reference and updated
    ...
    for (const auto& op : twoCharOps) {
        if (firstChar[0] == op[0] && pos < source.length() && source[pos] == op[1]) {
    ...
    switch(firstChar[0]) {
        ...
        default: return { TokenType::Unknown, firstChar };
    }
}